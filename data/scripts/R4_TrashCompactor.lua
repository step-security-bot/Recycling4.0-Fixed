...
local SortedGarbageExportContentsNr = 0
local soundTimer = 0
...
function toggleMaxStackSizeClicked()
	...
	this.Tooltip = {"tooltip_R4_TrashCompactor",MaxStackSize,"M",InQueue,"Q"}
end

function PutGarbageInQueue(SlotNr)
	QueuedStack = Find(this,"Stack",2)												-- check if there is a stack of garbage at specified slot
	...
	local MyGarbage = Find(this,"R4_CompactGarbage",2)
...
function SpawnSortedGarbageExports()
	myExportStack = Object.Spawn("Stack",this.Pos.x+this.Or.x,this.Pos.y+this.Or.y)
	if SortedGarbageExportContentsNr == 0 then
		FindStackNr(myExportStack)
	else
		myExportStack.Contents = SortedGarbageExportContentsNr
		myExportStack.Quantity = MaxStackSize
	end
	myExportStack.Tooltip = "tooltip_R4_SortedGarbageExport"
...
function FindStackNr(theStack)
	SortedGarbageExportContentsNr = 0											-- find out the contents number of R4_SortedGarbageExport stack
	Set(theStack,"Quantity",MaxStackSize)
	while not FoundContents do
		SortedGarbageExportContentsNr = SortedGarbageExportContentsNr+1
		Set(theStack,"Contents",SortedGarbageExportContentsNr)				-- stack contents is set by a number...
		if theStack.Contents == "R4_SortedGarbageExport" then					-- ...but reads out as a string
			FoundContents = true
		end
	end
end
